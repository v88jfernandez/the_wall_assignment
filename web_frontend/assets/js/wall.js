...
submitCreateMessageForm = (event) => {
    ...
    if(!create_message_form.querySelectorAll(".error_input").length){
        ...
        message_clone.setAttribute("data-message-id", generateRandom());
        
      
        create_message_form.reset();

        /* Event listeners for newly added message */
        ...
        message_clone.querySelector(".delete_message_form").addEventListener("submit", submitDeleteMessageForm);
...
submitCreateCommentForm = (event) => {
    ...
    if(!create_comment_form.querySelectorAll(".error_input").length){
        ...
        comment_clone.setAttribute("data-comment-id", generateRandom());
        
        create_comment_form.reset();

        /* Event listeners for newly added comment */
        ...
        comment_clone.querySelector(".delete_message_form").addEventListener("submit", submitDeleteMessageForm);
        ...
        item_container.querySelector(".responses").innerText = item_container.querySelectorAll(".comments_list li").length + " Responses";
    }
}
...
submitUpdateMessageForm = (event) => {
    ...
    paragrap.innerText = update_message_form.querySelector("textarea").value;
    update_message_form.replaceWith(paragrap);
